...
function parseInterval(interval: string): number {
	...
	if (!match) {
		throw new Error(
			`Invalid interval format: ${interval}. Use format like 60s, 5m, 1h`,
		);
	}
		...
		try {
			...
			if (result.failed.length > 0) {
				...
				if (stats.consecutiveErrors >= 3) {
					...
					logger.warn(
						`Multiple consecutive errors. Applying backoff: ${backoffDelay}ms`,
					);
				...
				if (stopOnError) {
					logger.error(
						"Stopping watch mode due to errors (--stop-on-error flag)",
					);
					process.exit(1);
				}
			} else {
				...
				if (result.synced.length > 0 || result.conflicts.length > 0) {
					logger.info(
						`  Synced: ${result.synced.length}, Conflicts: ${result.conflicts.length}`,
					);
				} else {
